...
func (s *Server) Start() error {
	...
	go func() {
		...
		for {
			...
			client.Id = utils.NewUUID(conn.RemoteAddr.String())
...
func (s *Server) SendUDP(client *Client, p packet.Packet) error {
	return nil
}
...
func (s *Server) Stop() {
	...
	go func() {
		// send tons of stop messages, so anyone listening gets one
		for {
			s.stop <- struct{}{}
		}
	}()
